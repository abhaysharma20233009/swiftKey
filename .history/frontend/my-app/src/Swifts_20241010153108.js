...
function Swifts() {
    ...
    const chunkedWords = chunkWords(words, 10); // Chunk words into groups of 10
    ...
    return (
        <div style={{ textAlign: 'center', marginTop: '20px' }}>
            ...
            />
            <div style={{ fontSize: '24px', marginTop: '10px', textAlign: 'left' }}>
                {chunkedWords.map((line, lineIndex) => (
                    <div key={lineIndex} style={{ display: 'flex', flexWrap: 'wrap' }}>
                        {line.map((word, index) => {
                            const wordIndex = lineIndex * 10 + index; // Calculate the global index
