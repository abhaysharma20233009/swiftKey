...
function Results() {
    ...
    const wordTyped = location.state?.currentWordIndex || 0; // Get currentWordIndex from state
                ...
                {info.map((msg) => (
                    msg.words_typed.map((wordObj, index) => (
                        <li key={`${msg._id}-${index}`}>
                            <strong>{wordObj.word}</strong> {/* Display each word typed on a new line */}
                            <p>{new Date(wordObj.timestamp).toLocaleString()}</p>
                        </li>
                    ))
                ))}
            </ul>
