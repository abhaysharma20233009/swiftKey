    ...
    useEffect(() => {
        if (time > 0) {
            const timer = setTimeout(() => {
                navigate('/results', { state: { currentWordIndex,username } }); // Pass currentWordIndex to results
            }, time * 1000);
        
            return () => clearTimeout(timer);
        }
    }, [time, currentWordIndex, navigate,username]); // Include currentWordIndex and navigate
    function Login(){
