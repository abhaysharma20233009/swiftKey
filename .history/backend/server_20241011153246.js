...
app.post("/login", async (req, res) => {
    try {
        ...
        req.session.user = { id: userRecord._id, userName: userRecord.userName, email: userRecord.email };
        console.log("Session created:", req.session.user);
        return res.status(200).json({ message: "User logged in successfully" }); // Changed to 200
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: "Internal Server Error" });
    }
...
app.post('/signup', async(req,res)=>{
    try {
        ...
        req.session.user = { id: newSignup._id, userName: newSignup.userName, email: newSignup.email };
...
app.delete('/signup', async(req,res)=>{
    try {
        const { userName, email, password } = req.body;
        const userRecord = await Key.findOneAndDelete({ userName, email });
