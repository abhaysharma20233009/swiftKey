...
function Swifts() {
    ...
    const [isInputDisabled, setIsInputDisabled] = useState(false); // New state
    ...
    const fetchMessages = async () => {
        try {
            ...
            if (data && typeof data === 'string') {
                ...
                setIsInputDisabled(false); // Enable input when fetching new messages
            } else {
    ...
    useEffect(() => {
        if (countdown > 0) {
            ...
            setIsInputDisabled(true); // Disable input when countdown reaches zero
        }
    }, [countdown]);
